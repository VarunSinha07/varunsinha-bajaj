      ...
      {Object.entries(tree).map(([node, children]) => (
        <li key={node} className="text-gray-300 font-medium relative">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center px-4 py-1.5 rounded-lg text-sm font-bold bg-gradient-to-r ${colorClass} text-white shadow-lg ring-1`}
            >
              {node}
            </span>
          </div>
...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      parsedData = parsedJson.data;
    } catch {
      setError(
        'Invalid JSON format. Please ensure your input is a JSON object with a "data" array of strings, e.g., { "data": ["A->B"] }.',
      );
      setLoading(false);
      return;
    }
      ...
      <main className="max-w-6xl mx-auto space-y-12">
        ...
        <header className="text-center space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto mt-4">
            A high-performance algorithmic pipeline transforming raw directed
            edge models into structured, cycle-detected trees.
          </p>
        </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <p className="text-xs text-gray-500 font-mono">
                Accepts JSON object with a &quot;data&quot; array. Evaluates
                cycles and filters multiple parents gracefully.
              </p>
              <button
                type="submit"
                disabled={loading}
                className={`relative px-8 py-3.5 rounded-xl font-bold text-white transition-all duration-300 shadow-xl overflow-hidden group ${
                  loading
                    ...
                    : "bg-gradient-to-r from-cyan-600 to-indigo-600 hover:scale-[1.02] hover:shadow-cyan-500/25 active:scale-[0.98]"
                }`}
              >
                {!loading && (
                  <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-cyan-400 to-indigo-400 opacity-0 group-hover:opacity-20 transition-opacity duration-300" />
                )}
          ...
          <section className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div
                  key={i}
                  className={`bg-white/5 backdrop-blur-md p-5 rounded-2xl border border-white/5 shadow-lg transition-all duration-300 ring-1 ring-transparent ${stat.ring}`}
                >
                  <span className="text-[0.65rem] text-gray-500 uppercase tracking-widest font-bold block mb-1">
                    {stat.label}
                  </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6 shadow-xl relative overflow-hidden">
                  ...
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-6 border-b border-white/10 pb-3">
                    Computation Metrics
                  </h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div className="bg-black/30 p-4 rounded-xl border border-white/5 text-center relative overflow-hidden transition hover:bg-black/50">
                      ...
                      <div
                        className={`text-4xl font-black mb-1 ${response.summary.total_cycles > 0 ? "text-rose-500" : "text-gray-400"}`}
                      >
                    ...
                    {response.invalid_entries.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {response.invalid_entries.map((entry, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center px-3 py-1 rounded-md text-xs font-mono bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.1)]"
                          >
                    ...
                    {response.duplicate_edges.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {response.duplicate_edges.map((edge, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center px-3 py-1 rounded-md text-xs font-mono bg-amber-500/10 text-amber-400 border border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.1)]"
                          >
                            {edge}
                          </span>
                        ))}
                ...
                <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden h-full flex flex-col">
                  ...
                  <div className="px-6 py-4 border-b border-white/10 bg-black/40 flex items-center justify-between">
                    ...
                    <span className="text-xs font-mono bg-white/10 px-2.5 py-1.5 rounded text-gray-300 border border-white/10 shadow-inner">
                      Trees & Cycles: {response.hierarchies.length} Output
                      Groups
                    </span>
                  </div>
                    ...
                    {response.hierarchies.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-4">
                        ...
                        <p className="font-medium text-lg text-gray-400">
                          Zero Logical Trees Formed
                        </p>
                        <p className="text-sm text-gray-600">
                          Ensure endpoints connect properly without total
                          disassociation.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-12 pb-10">
                        {response.hierarchies.map((hier, index) => (
                          <div
                            key={index}
                            className="relative group section-node"
                          >
                            {/* Core Head (Root Component) */}
                            <div className="flex items-center gap-4 mb-6">
                              <div
                                className={`p-1.5 rounded-xl border ${hier.has_cycle ? "bg-rose-500/10 border-rose-500/20" : "bg-cyan-500/10 border-cyan-500/20"}`}
                              >
                                <div
                                  className={`w-12 h-12 rounded-lg flex items-center justify-center font-black text-2xl shadow-lg ${hier.has_cycle ? "bg-rose-500/90 text-white" : "bg-cyan-500/90 text-white"}`}
                                >
                                  {hier.root}
                                </div>
                              </div>
                            ...
                            <div className="pl-6 pt-1">
                              {hier.has_cycle ? (
                                <div className="ml-2 inline-block px-5 py-3.5 bg-red-950/40 border border-rose-500/20 rounded-xl text-sm font-mono text-rose-400/80 italic shadow-inner">
                                  ? Aborted rendering to prevent infinite client
                                  loop overflow.
                                </div>
